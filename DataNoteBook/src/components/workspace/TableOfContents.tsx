...
//   handleUpload: (file: File, fromSidebar: boolean, folderId?: string | null) => void;
//   onClose: () => void;
//   onFileDoubleClick: (filename: string, fileId: string) => void; // ← updated
//   onRefresh?: () => void;
//   onCreateFolderClick?: () => void;
//   selectedFolderId?: string | null;
//   onSelectFolder?: (folderId: string) => void;
...
//                           <div
//                             key={file.id}
//                             onDoubleClick={() => onFileDoubleClick(file.name, file.id)} // ← updated
//                             className="flex items-center gap-1.5 px-2.5 py-0.5 cursor-pointer text-slate-300 hover:text-white hover:bg-slate-800/50 rounded-sm transition-colors text-[11.5px] select-none"
//                             title="Double click to preview dataset"
//                           >
//                             <FileSpreadsheet className="h-3.5 w-3.5 flex-shrink-0 text-emerald-500/70" />
...
export default function TableOfContents({ 
  ...
  const handleDownloadFile = async (fileId: string, fileName: string) => {
    try {
      ...
      const res = await fetch(`https://qual-be.hcloud.q2labs.ai/vault_resources/${fileId}/download`, {
        headers: token ? { "Authorization": `Bearer ${token}` } : {},
      });
      const data = await res.json();
      if (!res.ok || (data && data.success === false)) {
        throw new Error(data?.message || "Failed to fetch download details");
      }
      ...
      // Try to fetch blob directly first to trigger prompt download, 
      // but fallback to direct opening if fetch fails (e.g. CORS).
      try {
        const fileRes = await fetch(blobUrl);
        if (!fileRes.ok) throw new Error("CORS or fetch error");
          ...
          <div className="flex flex-col gap-0.5">
                      ...
                      {folderFiles.length === 0 ? (
                        ...
                        folderFiles.map((file) => (
                          <div
                            ...
                            className="flex items-center justify-between px-2.5 py-0.5 cursor-pointer text-slate-300 hover:text-white hover:bg-slate-800/50 rounded-sm transition-colors text-[11.5px] select-none group/file"
                          ...
                          >
                            ...
                            <button
                              ...
                              className="opacity-0 group-hover/file:opacity-100 p-0.5 hover:bg-slate-700/80 rounded transition-all text-slate-400 hover:text-slate-200 cursor-pointer"
                            >
            ...
            {rootFiles.map((file) => (
              <div
                ...
                onDoubleClick={() => onFileDoubleClick(file.name, file.id)}
                className="flex items-center justify-between px-6 py-1 cursor-pointer text-slate-300 hover:text-white hover:bg-slate-800/40 rounded transition-colors text-[12px] select-none group/file"
                title="Double click to preview dataset, drag to chat to analyze"
              >
                <div className="flex items-center gap-1.5 min-w-0 flex-1">
                  <FileSpreadsheet className="h-3.5 w-3.5 flex-shrink-0 text-emerald-500/70" />
